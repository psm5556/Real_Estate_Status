...
async function fetchJeonseRateData(params: {
  startWeek: string;
  endWeek: string;
  regionCode: string;
  ...
  url.searchParams.set("pIndex", "1");
  url.searchParams.set("pSize", "1000");
  url.searchParams.set("CLS_ID", params.regionCode);
...
export const getCachedJeonseRateData = unstable_cache(
  async (params: { startWeek: string; endWeek: string; regionCode: string }) =>
    fetchJeonseRateData(params),
  ["reb-jeonse-rate"],
  { revalidate: CACHE_TTL_SECONDS }
);
